...
let currentChapterId = "limits";

const STORAGE_PREFIX = "CalculusQuizHistory_";

// ================== MathJax 渲染函数 ==================
async function renderMath(container) {
    if (!window.MathJax) {
        console.warn("MathJax 尚未加载");
        return;
    }
    try {
...
async function revealQuizAnswer() {
    ...
    if (!explanationText.trim()) {
        ...
        answerDiv.innerHTML = `
            <div style="font-weight:500; margin-bottom:0.5rem;">📖 解释：</div>
            <div style="white-space:pre-wrap; line-height:1.6;">${escapeHtml(explanationText)}</div>
        `;
    }
...
// ================== 当前选中知识点（已修复公式显示） ==================
async function updateCurrentNodeInfo(nodeName) {
...
async function revealExplanation() {
    if (!currentSelectedNode) {
        const explanationDiv = document.getElementById('explanationText');
        if (explanationDiv) explanationDiv.innerHTML = `<div class="placeholder-text">⚠️ 尚未选中任何知识点</div>`;
        return;
    }
    ...
    await renderMath(explanationDiv);
}
...
function showHistoryForNode(node) {
    ...
    if (records.length === 0) {
        ...
        records.forEach(rec => {
            ...
            html += `<tr><td>${dateStr}</td><td><span class="${badgeClass}">${badgeText}</span></td></tr>`;
        });
        html += `</tbody></table>`;
